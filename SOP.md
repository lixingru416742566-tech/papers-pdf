...
每日自动搜索 arXiv 论文，按四个研究方向分类，**每方向限 10 篇**，去重检查，生成 JSON 数据 + Markdown 报告，推送到 GitHub 展示。
...
|------|------|
| **每方向限 10 篇** | 避免超时，保证质量 |
| **去重检查** | 基于 arxiv_id，检查今日 + 近 7 天数据 |
| **一句话中文概况** | 不调用 LLM，从标题/摘要提取 |
| **执行时间** | 每方向 <60 秒，总计 <5 分钟 |
